...
const CampaignDetails = () => {
  ...
  const [donorEmail, setDonorEmail] = useState(""); // ✅ Track donor's email
  ...
  const handleDonate = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/campaigns/${id}/donate`, {
        ...
        body: JSON.stringify({ amount: donationAmount, donorName, donorEmail }), // ✅ Include donorEmail
      });
      ...
      // Reset input fields after successful donation
      setDonationAmount("");
      setDonorName("");
      setDonorEmail("");
    } catch (err) {
              ...
              <ul className="mt-2">
                {campaign.donations.map((donation, index) => (
                  <li key={index} className="border-b py-2">
                    ...
                    {" "}({donation.donorEmail}) {/* ✅ Show email */}
                  </li>
                ))}
              </ul>
            ) : (
